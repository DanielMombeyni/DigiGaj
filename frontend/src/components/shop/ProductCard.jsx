...
export function ProductCard({ product, index = 0 }) {
  ...
  const discount =
    ...
    product.compare_at_price_toman > displayPrice
      ? Math.round(
          (1 - displayPrice / product.compare_at_price_toman) * 100,
        )
      : 0
    ...
    >
      <Link to={`/products/${product.slug}`} className="relative block aspect-[4/3] overflow-hidden bg-gradient-to-br from-ink-950 via-ink-900 to-sea-600/40">
        {product.primary_image ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 p-6 text-center">
            <span className="font-display text-3xl font-bold text-white/90">{product.name?.slice(0, 1)}</span>
            <BrandLogo size="xs" accentClass="text-white/50" restClass="text-white/50" />
          </div>
        )}
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-ink-950/50 via-transparent to-transparent opacity-80" />
        {discount > 0 && (
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <Link to={`/products/${product.slug}`}>
          <h3 className="mt-1 font-display text-base font-bold leading-7 text-ink-900 transition group-hover:text-copper-600 line-clamp-2">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-4">
          <div>
            <div className="font-bold text-copper-600">
              {product.has_options && product.min_price != null && product.min_price !== product.price_toman
